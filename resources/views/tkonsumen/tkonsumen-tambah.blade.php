            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light"></span>Tambah Data Transaksi
                            Konsumen
                        </h4>

                        <div class="card mb-4">
                            <div class="card-body">
                                <form action="/tkonsumen-insert" method="post">
                                    @csrf
                                    <div class="mb-3">
                                        ...
                                        <div class="form-group">
                                            <select class="form-control" name="id_penukaran" id="id_poin">
                                                ...
                                                @foreach ($penukarans as $p)
                                                    <option value="{{ $p->id }}"
                                                        data-poin="{{ $p->poins->poin }}">ID {{ $p->id_poin }} -
                                                        {{ $p->poins->poin }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" name="total" maxlength="50" required="required"
                                            class="form-control" id="total_pembayaran" readonly />
                                    </div>
                                    <div class="mb-3">
                                ...
                                <script>
                                    ...
                                    document.getElementById('id_poin').addEventListener('change', function() {
                                        var selectedOption = this.options[this.selectedIndex];
                                        var poin = parseFloat(selectedOption.getAttribute('data-poin'));
                                        var layananHarga = parseFloat(document.getElementById('total_pembayaran').value);

                                        if (poin === 0) {
                                            // Jika poin yang dipilih adalah "Tidak Mendapatkan Poin", harga tetap sama
                                            document.getElementById('total_pembayaran').value = layananHarga;
                                        } else {
                                            var newTotal = layananHarga - poin;
                                            document.getElementById('total_pembayaran').value = newTotal;
                                        }
                                    });
                                </script>
